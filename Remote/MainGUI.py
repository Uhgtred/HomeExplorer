...
class MainGUI(QDialog):
    ...
    def start2ndVideo(self):
        while True:
            ...
            imageUpdatingProcess = multiprocessing.Process(target=self.upDateImageLabel, args=(self, self.imgFrame, img), name='ImageUpdatingProcess')  # probably won't work in a process but needs to be done in a thread
            imageUpdatingProcess.start()
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break

    def upDateImageFrame(self, frame,  image):
        frame = cv2.imshow('Vision', image)
    ...
    def displayImage(self):
        ...
        imageUpdatingProcess = multiprocessing.Process(target=self.upDateImageLabel, args=(self, self.imgLabel, outImage), name='ImageUpdatingProcess') # probably won't work in a process but needs to be done in a thread
        imageUpdatingProcess.start()
        # self.imgLabel.setScaledContents(True)

    def upDateImageLabel(self, label,  image):
