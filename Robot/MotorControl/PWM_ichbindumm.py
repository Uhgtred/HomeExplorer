...
class PWM:
    ...
    def lChain(self, value, reverse):
        ...
        __reverse = reverse
        if __reverse:
            self.__channelLF.ChangeDutyCycle(0)  # making sure track stands before reward-action
            self.__channelLR.ChangeDutyCycle(__value)
        else:
            self.__channelLR.ChangeDutyCycle(0)  # making sure track stands before forward-action
            self.__channelLF.ChangeDutyCycle(__value)
            
    #could be a more modular function:
    #def track(self, pinF, pinR, value, reverse=False):
    ...
    #   __pinR = pinR
    #	if __reverse:
    #        __pinF.ChangeDutyCycle(0)  # making sure track stands before reward-action
    #        __pinR.ChangeDutyCycle(__value)
    #   else:
    #        __pinR.ChangeDutyCycle(0)  # making sure track stands before forward-action
    ...
    def rChain(self, value, reverse):
        ...
        __reverse = reverse
        if __reverse:
            self.__channelRF.ChangeDutyCycle(0)  # making sure track stands before reward-action
            self.__channelRR.ChangeDutyCycle(__value)
        else:
            self.__channelRR.ChangeDutyCycle(0)  # making sure track stands before forward-action
